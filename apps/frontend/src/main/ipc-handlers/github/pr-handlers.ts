...
/**
 * Sanitize content for inclusion in HTTP requests to GitHub
 *
 * This prevents potential injection attacks by sanitizing user-controlled
 * content that will be sent to GitHub's API. File paths and code snippets
 ...
 *
 * @param content - The content to sanitize (file paths, code snippets, etc.)
 * @returns Sanitized content safe for inclusion in HTTP request bodies
 */
...
/**
 * Sanitize a file path for safe display in GitHub comments
...
function sanitizeFilePath(filePath: string): string {
  ...
  // This prevents leaking full local directory structure
  const parts = filePath.split(/[/\\]/);
  if (parts.length <= 2) {
    // Already a simple filename or minimal path
    return filePath;
  }
      ...
      const postResult = await withProjectOrNull(projectId, async (project) => {
        ...
        try {
          ...
          if (options?.forceApprove) {
            ...
            body += `**Summary:** ${sanitizeForHttp(result.summary)}\n\n`;
    ...
    async (_, projectId: string, prNumber: number, body: string): Promise<boolean> => {
      ...
      const postResult = await withProjectOrNull(projectId, async (project) => {
        try {
          ...
          const tmpFile = join(project.path, ".auto-claude", `tmp_comment_body_${randomBytes(16).toString("hex")}.txt`);
          try {
