...
function debugLog(message: string, data?: unknown): void {
  if (DEBUG) {
    // Sanitize message to prevent log injection via template literals
    const safeMessage = sanitizeForLog(message, 1000);
    if (data !== undefined) {
      // If data is a string, sanitize it before logging
      if (typeof data === 'string') {
        console.warn(`[GitHub OAuth] ${safeMessage}`, sanitizeForLog(data));
      } else {
        console.warn(`[GitHub OAuth] ${safeMessage}`, data);
      }
    } else {
      console.warn(`[GitHub OAuth] ${safeMessage}`);
    }
  }
}
